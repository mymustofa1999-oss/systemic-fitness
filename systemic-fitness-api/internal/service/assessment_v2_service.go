...
func (s *AssessmentV2Service) Submit(
	ctx context.Context, userID string, in *SubmitV2Input,
	...
	if s.trainerCardService != nil {
		if meta, err := s.trainerCardService.repo.GetActivationMetadata(ctx, userID); err == nil && meta != nil && meta.HasActiveSub {
			...
			if isManualCard {
				...
				s.logger.Info("[AssessmentV2.Submit] active subscription found, regenerating default training card", "user_id", userID)
