...
func main() {
	...
	for _, file := range levelFiles {
		...
		for _, row := range rows {
			...
			setName := ""
			if len(row) > 2 { setName = strings.TrimSpace(row[1]) + " " + strings.TrimSpace(row[2]) }
			index := ""
			...
			if err != nil {
				...
				err = conn.QueryRow(ctx, `
					...
					VALUES ($1, $2, $3, $4, $5)
					RETURNING id
				`, movementName, "full_body", maleURL, femaleURL, cats).Scan(&mID)
				...
				conn.Exec(ctx, "UPDATE dl_movements SET video_url_female=$1, video_url_male=$2, categories=array_append(array_remove(categories, 'fc'), 'cd') WHERE id=$3 AND NOT (categories @> ARRAY['cd']::varchar[])", femaleURL, maleURL, mID)
			}
			...
			if !exists {
				...
				_, err = conn.Exec(ctx, `
					...
					VALUES ($1, $2, $3, $4, $5)
				`, catID, levelID, mID, "full_body", sortOrder)
				if err != nil {
