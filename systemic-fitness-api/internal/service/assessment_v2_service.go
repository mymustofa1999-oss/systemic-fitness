...
func (s *AssessmentV2Service) Submit(
	...
	// Auto-create/regenerate the default training card on assessment submit, but
	// ONLY for system-managed cards. A card an admin/trainer set up manually must
	// always follow that client's own setup — never overwrite it on resubmission.
	if s.trainerCardService != nil {
		isStaff := callerRole == model.RoleAdmin || callerRole == model.RoleTrainer || callerRole == model.RoleOwner
		meta, err := s.trainerCardService.repo.GetActivationMetadata(ctx, userID)
		hasActiveSub := err == nil && meta != nil && meta.HasActiveSub

		if hasActiveSub || isStaff {
			existing, getErr := s.trainerCardService.repo.GetByCustomerID(ctx, userID)
			isManualCard := getErr == nil && existing != nil &&
				(existing.Notes == nil || !strings.Contains(*existing.Notes, autoGeneratedMarker))
			if isManualCard {
				s.logger.Info("[AssessmentV2.Submit] manual training card present, preserving it", "user_id", userID)
			} else {
				s.logger.Info("[AssessmentV2.Submit] regenerating default training card", "user_id", userID, "hasActiveSub", hasActiveSub, "isStaff", isStaff)
...
func (s *AssessmentV2Service) GetTrainingCard(ctx context.Context, userID string) (*model.TrainingCardResponse, error) {
	...
	if !isPaidActive {
		...
		//    (admin/trainer customised the movements) must never be overwritten.
		isAutoGenerated := dbCard.Notes != nil && strings.Contains(*dbCard.Notes, autoGeneratedMarker)
