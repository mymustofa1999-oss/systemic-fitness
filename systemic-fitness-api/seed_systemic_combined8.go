...
func main() {
	...
	for _, file := range levelFiles {
		...
		for i, row := range rows {
			...
			rowFVideo := getCol(idxFVideo)
			rowMVideo := getCol(idxMVideo)
			
			if fU == "" && fL == "" && mU == "" && mL == "" { continue }

			catCode := strings.ToLower(currentSeq)
			if catCode != "fc" && catCode != "cc" && catCode != "mc" && catCode != "cd" { continue }
			...
			// Link video from videoMap
			fVideo := femaleVideos[normalizeName(femaleName)]
			// Fallback: maybe they didn't put a hyphen in the video sheet
			...
			// Override with Level sheet links if present
			if rowFVideo != "" && strings.ToLower(rowFVideo) != "waitlist" { fVideo = rowFVideo }
			if rowMVideo != "" && strings.ToLower(rowMVideo) != "waitlist" { mVideo = rowMVideo }
			...
			if mov, exists := movements[movementName]; !exists {
				...
				if fVideo != "" { mov.VideoURLFemale = fVideo }
				if mVideo != "" { mov.VideoURLMale = mVideo }
			}
		}
		xl.Close()
	}
	...
	for _, file := range levelFiles {
		...
		for i, row := range rows {
			...
			var movID string
			err = conn.QueryRow(ctx, "SELECT id FROM dl_movements WHERE name = $1 LIMIT 1", movementName).Scan(&movID)
			if err != nil || movID == "" { continue }

			sortOrder++
			
			bodyPart := "whole body"
			if !isDynamic { bodyPart = inferBodyPart(currentType) }
			
			_, err = conn.Exec(ctx, `
				INSERT INTO dl_menu_items (category_id, level_id, movement_id, body_part, sort_order, set_name, group_type)
				VALUES ($1, $2, $3, $4, $5, $6, $7)
			`, catID, levelID, movID, bodyPart, sortOrder, currentSet, currentType)
			if err != nil {
